import { PER_TRANSECTION_FEE } from "@/constants/globals";
...
export default async function handler(req, res) {
    if (req.method === 'POST') {
        try {
            ...
            const { data } = await axios.get(`https://api.1inch.exchange/v3.0/137/quote?fromTokenAddress=${fromTokenAddress}&toTokenAddress=${toTokenAddress}&amount=${amount}&fee=${PER_TRANSECTION_FEE}`);
